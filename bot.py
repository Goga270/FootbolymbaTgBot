...
def player_list_to_display(session: Session, ids: List[int], sort_alphabetically: bool = True) -> str:
    ...
    return '\n'.join(f"- {escape(p.full_name or '')}" for p in players)
...
def draft_ordered_roster_to_display(session: Session, team_ids: List[int]) -> str:
    ...
    cap_name = escape(cap.full_name or '') if cap else f"ID {cap_id}"
    ...
    for idx, pid in enumerate(team_ids[1:], 1):
        ...
        p_name = escape(p.full_name or '') if p else f"ID {pid}"
...
def generate_admin_rosters_helper(session: Session, match: Match) -> str:
    ...
    for pid in team_a_ids:
        p = session.get(Player, pid)
        if p:
            text += f"• <code>{escape(p.nickname or '')}</code> — {escape(p.full_name or '')} (@{escape(p.tg_username or '') or 'нет'})\n"
...
async def match_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "Использование: <code>/match_result &lt;ID_матча&gt;</code>\n"
            "Пример: <code>/match_result 1</code>",
            parse_mode='HTML'
        )
        return
    ...
    try:
        ...
        await update.message.reply_text("❌ ID матча должен быть числовым.")
        return
    ...
    with SessionLocal() as session:
        ...
        if match.status != MatchStatus.FINISHED:
            await update.message.reply_text(
                f"⚠️ Матч №{match_id} еще не завершен (статус: <code>{match.status.value}</code>).\n"
                f"Результаты и статистика доступны только для завершенных игр.",
                parse_mode='HTML'
            )
            return
        ...
        ds = match.draft
        color_a = ds.team_a_color if (ds and ds.team_a_color) else "🔴"
        color_b = ds.team_b_color if (ds and ds.team_b_color) else "🔵"

        # Сбор голевых действий
        actions = session.query(Action).filter_by(match_id=match_id).all()
        actions_text = ""
        if actions:
            actions_text = "\n\n📊 <b>Результативные действия:</b>\n"
            players_actions = {}
            for action in actions:
                players_actions.setdefault(action.player_id, []).append(action)

            for player_id, act_list in players_actions.items():
                player = session.get(Player, player_id)
                goals = [a for a in act_list if a.action_type == 'goal']
                assists = [a for a in act_list if a.action_type == 'assist']

                parts = []
                if goals:
                    g_minutes = [f"{g.minute}'" for g in goals if g.minute is not None]
                    g_suffix = f" ({', '.join(g_minutes)})" if g_minutes else ""
                    parts.append(f"⚽ {escape(str(len(goals)))}{g_suffix}")
                if assists:
                    a_minutes = [f"{a.minute}'" for a in assists if a.minute is not None]
                    a_suffix = f" ({', '.join(a_minutes)})" if a_minutes else ""
                    parts.append(f"🎯 {escape(str(len(assists)))}{a_suffix}")

                actions_text += f"• <b>{escape(player.full_name)}</b> ({escape(player.nickname)}): {', '.join(parts)}\n"
        else:
            actions_text = "\n\n📊 <i>Статистика голов и пасов для этого матча не занесена.</i>"
        ...
        # Компактный вывод результатов без составов [2]
        msg = (
            f"🏁 <b>Результаты матча №{match.id}</b>{tour_info}:\n\n"
            f"{color_a} Команда А ({name_a})  <b>{match.score_a} - {match.score_b}</b>  {color_b} Команда Б ({name_b})\n"
            f"{actions_text}"
        )
...
async def finish_match(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as session:
        ...
        context.user_data['silent_stats'] = False
        roster_text = generate_admin_rosters_helper(session, match)

        await update.message.reply_text(
            f"✅ <b>Матч №{match_id} успешно завершен ({score_a}-{score_b}).</b>\n\n"
            f"Внимание: Результаты будут опубликованы в группе <b>после внесения всей статистики</b> и ввода <code>/done_actions {match_id}</code>!\n\n"
            f"{roster_text}",
            parse_mode='HTML'
        )
...
async def done_actions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    silent = context.user_data.pop('silent_stats', False)
    await update.message.reply_text(
        "✅ Запись голевых действий успешно завершена. Сводный результат отправлен в группу.")

    if TARGET_GROUP_ID and not silent:
        with SessionLocal() as session:
            match = session.get(Match, match_id)
            if not match:
                return
            ...
            congrats = f"🏆 Поздравляем команду <b>{escape(winner_team_name or '')}</b> с победой!" if score_a != score_b else "🤝 Боевая ничья!"
            ...
            if actions:
                ...
                for player_id, action_list in players_actions.items():
                    ...
                    actions_text += f"• <b>{escape(player.full_name or '')}</b> ({escape(player.nickname or '')}): {', '.join(parts)}\n"
            ...
            group_msg = (
                ...
                f"{color_a} Команда А ({escape(cap_a.full_name or '')})  <b>{score_a} - {score_b}</b>  {color_b} Команда Б ({escape(cap_b.full_name or '')})\n\n"
                f"{congrats}"
                f"{actions_text}"
            )
...
async def add_pairing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as session:
        ...
        if nick_a.lower() not in ['none', '?', 'tbd'] and not cap_a:
            await update.message.reply_text(f"❌ Капитан '{escape(nick_a or '')}' не найден в БД.")
            return
        if nick_b.lower() not in ['none', '?', 'tbd'] and not cap_b:
            await update.message.reply_text(f"❌ Капитан '{escape(nick_b or '')}' не найден в БД.")
            return
        ...
        status_text = f"обновлена со счетом <b>{escape(score_str or '')}</b>" if score_str else "успешно обновлена"
        name_a = escape(cap_a.full_name or '') if cap_a else "TBD"
        name_b = escape(cap_b.full_name or '') if cap_b else "TBD"

        await update.message.reply_text(
            f"✅ Ячейка Слот {slot_id} (Пара <b>{name_a} vs {name_b}</b>) {status_text} в турнире <b>{escape(t.name or '')}</b>.",
            parse_mode='HTML'
        )
...
def get_seeds_for_slot(slot_id: int, N: int) -> tuple[str, str]:
    """Возвращает стандартные номера посева для первого раунда в зависимости от размера сетки."""
    SEEDS_16 = {
        1: ("1", "16"),
        2: ("8", "9"),
        3: ("4", "13"),
    ...
    if N == 16:
        return SEEDS_16.get(slot_id, ("", ""))
    elif N == 8:
        return SEEDS_8.get(slot_id, ("", ""))
    elif N == 4:
        return SEEDS_4.get(slot_id, ("", ""))
    return ("", "")


async def view_bracket(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """(Для всех) Вывод сетки матчей турнира в виде красивой PNG-картинки (один в один с вашим фото) и текста."""
    if not await check_group_admin_permissions(update, context):
        return
    ...
    with SessionLocal() as session:
        ...
        # --- ГЕНЕРАЦИЯ ДВУСТОРОННЕЙ КАРТИНКИ (PILLOW) --- [2]
        try:
            import io
            import math
            ...
            col_gap = 100
            box_width = 240
            box_height = 80  # Увеличено до 80px для двухстрочного вывода

            img_width = 50 + (total_rounds * (col_width + col_gap)) + 150
            img_height = (N // 2) * 140 + 200
            if img_height < 600:
                img_height = 600

            img = Image.new('RGB', (img_width, img_height), color='#1e1e1e')  # Темно-серый фон
            ...
            # 1. Наносим фоновую точечную сетку (Dot Mesh) для стильного эффекта [2]
            for dx in range(0, img_width, 20):
                for dy in range(0, img_height, 20):
                    draw.point((dx, dy), fill='#2d2d2d')

            # Заголовок
            draw.text((40, 25), f"🏆 СЕТКА: {t.name.upper()} ({t.match_format})", fill='#eab308', font=font_title)
            ...
            # Координата матча за 3-е место
            x_coords[N] = img_width - col_width - 50
            y_coords[N] = img_height - 140

            # Вспомогательная функция обрезки имен для аккуратности
            def limit_name(name: str, max_chars: int = 16) -> str:
                return name[:max_chars - 3] + "..." if len(name) > max_chars else name

            # 2. РИСУЕМ СВЯЗУЮЩИЕ СКОБКИ (Bracket Hooks) ОДИН В ОДИН С ФОТО [2]
            for r_idx in range(len(rounds) - 1):
                r = rounds[r_idx]
                next_round = rounds[r_idx + 1]

                for i in range(r["count"] // 2):
                    slot_p1 = r["offset"] + 2 * i + 1
                    slot_p2 = r["offset"] + 2 * i + 2
                    next_slot_id = next_round["offset"] + i + 1
                    ...
                    x_hook = x_curr + 15
                    y_mid = (y_curr1 + y_curr2) / 2

                    # Рисуем фигурные ушки скобки "}"
                    draw.line([(x_hook, y_curr1), (x_hook, y_curr2)], fill='#666666', width=2)
                    draw.line([(x_hook, y_curr1), (x_hook - 15, y_curr1)], fill='#666666', width=2)
                    draw.line([(x_hook, y_curr2), (x_hook - 15, y_curr2)], fill='#666666', width=2)

                    # Рисуем указатель вправо
                    draw.line([(x_hook, y_mid), (x_hook + 15, y_mid)], fill='#666666', width=2)

                    # Пишем номер следующего слота рядом со скобкой
                    draw.text((x_hook + 22, y_mid - 7), str(next_slot_id), fill='#aaaaaa', font=font)

            # 3. РИСУЕМ ДВУХСТРОЧНЫЕ ЯЧЕЙКИ С ПОСЕВОМ И СЧЕТОМ [2]
            for p in pairings:
                slot_id = p.slot_id
                x = x_coords[slot_id]
                y = y_coords[slot_id]

                # Маленький индекс слота слева от коробки (как на фото)
                draw.text((x - 22, y + 33), str(slot_id), fill='#555555', font=font_small)
                ...
                # Разбираем и вытаскиваем раздельные счета
                ...
                border_color = '#eab308' if p.is_completed else '#444444'
                box_color = '#111111' if p.is_completed else '#222222'

                # Общая рамка ячейки [2]
                draw.rectangle([x, y, x + box_width, y + box_height], fill=box_color, outline=border_color, width=2)
                # Горизонтальный разделитель посередине [2]
                draw.line([(x, y + 40), (x + box_width, y + 40)], fill='#2d2d2d', width=2)

                # Получаем стандартные номера посева
                seed_a, seed_b = get_seeds_for_slot(slot_id, N)

                # --- СТРОКА А (Верхняя половина) ---
                # Маленький серый бокс посева слева [2]
                draw.rectangle([x, y, x + 35, y + 40], fill='#2d2d2d', outline='#353535', width=1)
                if seed_a:
                    draw.text((x + 13, y + 14), str(seed_a), fill='#888888', font=font_small)
                # Имя
                draw.text((x + 47, y + 12), limit_name(name_a, 16), fill='#f3f4f6', font=font)

                # --- СТРОКА Б (Нижняя половина) ---
                # Маленький серый бокс посева слева [2]
                draw.rectangle([x, y + 40, x + 35, y + 80], fill='#2d2d2d', outline='#353535', width=1)
                if seed_b:
                    draw.text((x + 13, y + 54), str(seed_b), fill='#888888', font=font_small)
                # Имя
                draw.text((x + 47, y + 52), limit_name(name_b, 16), fill='#f3f4f6', font=font)

                # --- ЯЧЕЙКИ СЧЕТА (Отрисовываются справа, только если матч сыгран) --- [2]
                if score_a_text or score_b_text:
                    # Бокс счета А
                    draw.rectangle([x + box_width - 45, y, x + box_width, y + 40], fill='#2d2d2d', outline='#353535',
                                   width=1)
                    draw.text((x + box_width - 28, y + 12), score_a_text, fill='#ffffff', font=font)
                    # Бокс счета Б
                    draw.rectangle([x + box_width - 45, y + 40, x + box_width, y + 80], fill='#2d2d2d',
                                   outline='#353535', width=1)
                    draw.text((x + box_width - 28, y + 52), score_b_text, fill='#ffffff', font=font)
...
async def log_match_found_cap_a(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await start_player_search(
        ...
        f"Капитан А: <b>{escape(cap_a.full_name or '')}</b>.\n\n"
...
async def log_match_found_cap_b(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"Капитаны: <b>{escape(cap_a.full_name or '')} vs {escape(cap_b.full_name or '')}</b>\n\nЭто был ответный матч (реванш)?"
...
async def add_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "Использование: <code>/add_stats &lt;ID_матча&gt;</code>\n"
            "Пример: <code>/add_stats 1</code>",
            parse_mode='HTML'
        )
        return
    ...
    with SessionLocal() as session:
        match = session.get(Match, match_id)
        if not match:
            await update.message.reply_text(f"❌ Match №{match_id} не найден в базе данных.")
            return

        if match.status != MatchStatus.FINISHED:
            await update.message.reply_text(
                f"❌ Ошибка: Матч №{match_id} еще не завершен.\n"
                f"Для завершения матча сначала используйте:\n"
                f"<code>/finish_match {match_id} СчетА-СчетБ</code>",
                parse_mode='HTML'
            )
            return
...
def build_app():
    ...
    app.add_handler(CommandHandler("match_result", match_result))
